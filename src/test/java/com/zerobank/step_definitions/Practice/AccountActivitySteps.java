...
public class AccountActivitySteps extends TestBase {
  ...
  public void results_table_should_only_show_descriptions_containing_OFFICE(String expected) {
    System.out.println(expected+": "+accountActivityPage.filterBColumn(3));
    System.out.println(expected+": "+accountActivityPage.filterBColumn(4));
  }
  ...
  public void results_table_should_show_at_least_one_result_under_Withdrawal(String expected) {
    BrowserUtils.wait(1);
    accountActivityPage.setTypeDrop(expected);
    accountActivityPage.find_bttn.click();
//    accountActivityPage.scrollToElement(accountActivityPage.filter_Res.get(1));
    waitForVisibility(accountActivityPage.Filter, 3);
    System.out.println(accountActivityPage.filterBColumn(3));
  }
//
  @When("user selects type {string}")
  public void user_selects_type(String string) {
    BrowserUtils.wait(1);
    accountActivityPage.setTypeDrop(string);
    accountActivityPage.find_bttn.click();
    BrowserUtils.wait(1);
  }

  @Then("results table should show no result under {string}")
  public void results_table_should_show_no_result_under_Withdrawal(String expected) {
//    System.out.println(accountActivityPage.filterBColumn(4));
//    System.out.println(accountActivityPage.filterBColumn(3));

    if (accountActivityPage.filterBColumn(3).isEmpty()) {
      System.out.println(accountActivityPage.filterBColumn(4));
      Assert.assertTrue(expected, accountActivityPage.filterBColumn(3).isEmpty());
    } else if (accountActivityPage.filterBColumn(4).isEmpty()) {
      System.out.println(accountActivityPage.filterBColumn(3));
      Assert.assertTrue(expected, accountActivityPage.filterBColumn(4).isEmpty());
    }
    BrowserUtils.wait(4);
  }
}